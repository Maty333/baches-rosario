...
export const verifyEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.redirect(`${frontendUrl}/login?error=token_invalid_or_expired`)
    }
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({
        message: 'Si existe una cuenta, se envió el correo de restablecimiento'
      })
    }
  ...
  try {
    ...
    try {
      ...
      if (tokenError.message && tokenError.message.includes('invalid_grant')) {
        return res.redirect(
          `${frontendUrl}/auth/google/callback?error=invalid_grant`
        )
      }
